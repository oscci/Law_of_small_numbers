...
<!--- Adam also wrote: I’d addressed the majority of points raised on the pre-reg a while back: https://docs.google.com/document/d/14BsWNgACJaSt7vebQma3N-otaLHFB5zsXsg4meFayVY/edit?usp=sharing. I have downloaded that file--->
...
dozoneplot<-function(oddsframe,ES,numrangex,myint){
  ...
  breakpoints<-c(-1000000,-log(80),-log(40),-log(20),-log(10),-log(4),log(4),log(10),log(20),log(40),log(80),1000000)
...
numrangex<-seq(10,320,2) #sequence of sample sizes to model; can modify this

for (ES in c(.1,.3,.5)){ #sequence of True effect sizes to model
  
  grain<-.025 #this will mean we compute probabilities for effect sizes using this as array size
...
colnames(strategies)<-c('strategy','meancorr','meanarray','meanreward')
...
# Pilot testing
The task was piloted with 21 people recruited via Prolific [Adam to add more here?]. 
Results are summarised in Table `r tabnumber`. With 80 items, a score of 47 (58.7%) correct indicates better than chance (binomial probability = .046). Six of the pilot participants did not perform above chance; one of these low performers did show a significant improvement across the two halves of the session, achieving 68% correct in the second half. The highest performing participant achieved 80%. Overall, the pilot data suggested the task might be too difficult to show learning, and it was accordingly decided to change the displays of distributions to include bars corresponding to the means for the two groups. Informal piloting with colleagues suggested this led to an increase in accuracy.  
...
These participants also completed the probability quizzes that preceded and followed the training session. If changes during training were meaningful, we might expect to see more improvement on Law of Small Numbers items for those participants who had shown signs of waiting longer to response over time. 




Here data is read in and prepared.
