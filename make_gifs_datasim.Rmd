...
myspread<-data.frame(matrix(nrow=(2+length(nsets)+sum(nsets)),ncol=60)) #rows need to allow for intro & end(3) breaks (length(nsets))+row for each trial (sum(nsets))
...
# save,
filename2<-paste0('gifs_pngs/E',filename,'.gif')
...
q <- q + theme_bw() + theme(legend.title = element_blank()) + theme(legend.position="top")  +
  theme(text = element_text(size=15))
q
filename3<-paste0('gifs_pngs/E',filename,'.png')
