...
# Modified by DB Feb 2021 to add LL and Bayes Factor information
...
# For the Gorilla game, we need gifs that show how a dataset changes as additional datapoints are sampled. 
# This script creates gifs. We also create the spreadsheet that will be used to control display of trials 
...
myspread<-data.frame(matrix(nrow=(2+Nblock+sum(nsets)),ncol=58)) #rows need to allow for intro and breaks
...
for (effsize in 1:length(Elist)){
  ...
  for (i in 1:ntrial){
    ...
    if(gifmake==1){
      ...
      #p
      ggsave(paste0('static jpg versions/myplot',E,'_',num2alpha(i),'.jpg'),width= 7, height = 7,p)
      ...
      filename<-paste0('gifs_pngs/E',Elist[effsize],'_',num2alpha(i),'.png')
