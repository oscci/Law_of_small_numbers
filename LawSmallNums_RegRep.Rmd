...
## Abstract
The term 'law of small numbers' was coined in 1971 by Tversky and Kahneman, who noted that even statistically-trained people were poor at judging how variability of mean estimates declined with sample size. This cognitive bias, which we term 'sampling variation blindness' can make it difficult to gain an intuitive appreciation of statistical power. In this study, we consider whether training with simulated datasets could help people overcome this bias. Up to 156 people will be recruited for an online training session with 80 trials, each of which involves viewing a gif that displays beeswarm plots from two samples that doubles in size in six steps, with arrays going from 10 to 320 per group. They are told that the pink group is a control with mean 0 and SD of 1, and the blue group is a comparison group drawn either from the same population as the pink group, or from a population with mean effect size of .3. Their task is to respond either 'blue = pink' or 'blue > pink' as soon as they are confident. They have feedback on each trial, earning points for correct responses and losing them for incorrect responses. Before and after training, subjects are given parallel forms of a 12-item quiz testing knowledge of statistics and probability, including six items testing whether they show sampling variation blindness. We hypothesise that explicit exposure to displays that illustrate how sample size affects mean estimates will counteract sampling variation blindness. Accordingly, we predict that subjects' gains in accuracy during training will be associated with waiting to respond until larger array sizes, and those who show such gains will also show improvement specifically on quiz items relating to sampling variation blindness. 
...
jpeg(plotname, width = 600, height = 350)
par(mfrow=c(1,2))
for (i in c(10,100)){
...
We will also examine the potential of this task to enhance subjects' statistical reasoning by administering a pre- and post-task quiz testing statistical reasoning using both conventional probability questions and questions that assess sampling variation blindness.  

Specific hypotheses are:  
Hypothesis 1: In line with early work by Tversky and Kahneman (1971), people will be overconfident about estimates based on small samples.  We predict that we will demonstrate this in the statistics quiz, and also with our new task, showing that typically people make decisions based on samples where the number of data points is too small to give clear evidence.  
...
Hypothesis 3: Learning will generalize beyond the training task. Individual differences in learning on the training task will predict improvement on a statistical quiz, specifically for items that are designed to assess sampling variation blindness.   

# Method

## Subjects
We will recruit up to 156 subjects via the online research platform Prolific and social media platforms. Individuals will complete the statistics quiz and judgement task online at a time and place of their choosing. The study will be implemented using Gorilla, a cloud-based research tool for behavioural studies. Subjects will be paid £5 for their time; the study will take approximately 30 minutes to complete.  
...
All items included a pink sample from a population with mean of zero and SD of one, plus a blue sample. For the blue sample, half the items were drawn from the same population as the pink sample (i.e. null effect) and half were drawn from a population with mean of 0.3 and SD of one (i.e., true effect). Figure 3 illustrates a set of samples shown side by side for a trial with a true effect.  The observations were depicted in a beeswarm plot in either pink or blue. Items for pilot testing showed only the datapoints corresponding to the two datasets, but, as discussed below, for the current experiment, we added the sample mean shown as a horizontal bar.  `r fignumber<-fignumber+1`
...
On the basis of the power analysis, we could just do a simple count of how commonly subjects responded when the array index was less than 4, i.e. when the chances of detecting a true effect were below .5. However, the trial by trial variation allows us to do a rather more sensitive analysis, testing how the evidence available on a specific trial is used.`r fignumber <- fignumber+1`  Figure `r fignumber` illustrates the logic. The figure shows how evidence accumulates over a series of trials. 
...
This hypothesis will be tested using data both from the Pretest statistical quiz and the training task. On the pretest statistical quiz, it is expected that the majority of subjects will fail to select the correct response on items that assess awareness of the importance of sample size, and when they do so, will either respond that sample size is not important, or, when asked to select an optimal sample size, will select one that is smaller than required.  
In the training task, we predict that responses in the first half of training will tend to be non-optimal, made when the array size is small enough to make an error likely. The array index at which a response is made for all subjects will be compared with a value of 5, using a directional one-sample t-tets, with the prediction that most participants will make responses at earlier array indices than this.
...
Here the prediction is that individual differences in learning on the training task will predict improvement on the statistical quiz, specifically for items that are designed to assess sampling variability blindness. 

<!---I'm still struggling here with optimal analysis.--->
A linear mixed model will be used, whereby each quiz item is coded as P or S, and each subject's response coded as 0 or 1. The dependent measure is the response to post-test quiz items, and predictors are response on pre-test items, and percentage correct on the first and second halves of training. It is predicted that, for S-items, there will be an improvement in scores from pre-test to post-test.

Further exploratory analysis will be used to determine whether individual differences in training effects are related to improvement in performance on S-items. In addition, the response profiles on the training task will be scrutinised for those subjects who do show learning, with the aim of determining whether they are sensitive to log likelihood, or whether they simply learn a strategy of waiting for a larger array size.   
...
### Initial queries (session 1 only)

i. How confident are you in interpreting the results of a t-test?  
a) Very confident  
b) Fairly confident  
c) Not confident  
d) Very little idea about what a t-test is  
  
ii. How familiar are you with the idea of statistical power?  
a) Very familiar  
