...
Compared to laypeople, scientists receive extensive training to help them understand and appropriately address the uncertainty of evidence. Yet many scientists fall short in their understanding of statistical concepts and experimental design — an underlying failure to appreciate uncertainty. One cognitive bias demonstrated by @tversky1971 is the 'belief in the law of small numbers'. This refers to the tendency to overestimate the reliability of data that come from small samples - which we shall term 'sampling variation blindness'. For instance, as shown in Figure 1, if repeatedly sampling `r samplesizes[1]` men from a population, the mean height of the sample will be far more variable than when repeatedly sampling `r samplesizes[2]` men. People understand that sample size does not affect the expected mean value, but they tend not to appreciate that it has a large effect on the standard error of the mean (i.e. variability of the red bars). This has implications for understanding of statistical power, and can help explain why so many studies in psychology, and indeed many other scientific disciplines, are underpowered. 

![Figure `r fignumber`. Six independent samples of simulated male height, each with sample size of 10 or 60. Each point represents height of one male, drawn from population with mean 178 cm and SD 10 cm, and the red bar is the mean for that sample. The horizontal blue line represents the population mean.](Fig1.jpg)

In 1962, @cohen1962 embarked on a project of improving psychologists' understanding of statistical power, providing tools to help people compute power and documenting the extent of underpowered studies in social psychology, with the aim of reducing waste in research efforts. However, 27 years later, @sedlmeier1989 reported that things had not changed at all. And in 2016, similar conclusions were drawn from a large review of studies in social and behavioral sciences up to 2011 (@smaldino2016). In 2018 a review concluded that low power remains a major factor explaining poor replicability of highly cited studies in psychology (@stanley2018). A wide range of areas are affected, from neuroscience (@poldrack2017), to infancy research (@oakes2017). Why, despite years of attempts to improve the dismal record of low power in psychology, do researchers persist in performing underpowered studies? We suspect the explanation may go beyond lack of training, and reflect the influence of sampling variability blindness, which leads us to have intuitions about sample size that are at odds with reality. Consider this example (@tversky1971, p.105):

_‘Suppose you have run an experiment on 20 subjects, and have obtained a significant result which confirms your theory (z= 2.23, p < .05, two-tailed). You now have cause to run an additional group of 10 subjects. What do you think the probability is that the results will be significant, by a one-tailed test, separately for this group?_
...
We will also examine the potential of this task to enhance subjects' statistical reasoning by administering a pre- and post-task quiz, testing statistical reasoning using questions that assess sampling variation blindness.  The quiz also includes questions testing understanding of basic probability; these act as positive controls: if subjects score below 66% on this set of questions, then this would indicate they either have little training in statistics, or are not taking the task seriously.

Specific hypotheses are:  
_Hypothesis 1_: In line with early work by Tversky and Kahneman (1971), people will be overconfident about estimates based on small samples.  We predict that we will demonstrate this in the estimation quiz, where performance will be poor on questions that focus on sampling variation blindness, and also with our training task, showing that typically people make decisions based on samples where the number of data points is too small to give clear evidence.  
_Hypothesis 2_: Repeated exposure to a training task with feedback can reduce overconfidence in small samples. Accordingly, we predict that, in the course of training, a proportion of subjects will start to wait longer to view evidence from larger arrays, and correspondingly achieve higher levels of accuracy.  
_Hypothesis 3_: Learning will generalize beyond the training task. Individual differences in learning on the training task will predict improvement on sampling variation blindness items on the estimation quiz. 
...
## Subjects
Our subject inclusion criteria include: (1) age of 18 years or over, (2) have studied life or social sciences for at least one term at undergraduate (bachelor’s degree) level. We will recruit up to 120 subjects via the online research platform Prolific (www.prolific.co) and social media platforms. The maximum sample size is determined from a power calculation (see section on Simulated data for sample size determination, below) indicating that this is sufficient to detect the case when 33% or more of subjects shift from reliance on small arrays at start of training to arrays that are meaningfully larger by end of training. 

## Procedure  
After providing informed consent, subjects will complete the estimation quiz and judgment task online at a time and place of their choosing, followed by an alternative form of the post-test estimation quiz. Subjects are paid £7.50 for their time, but can earn a further bonus linked to the number of correct items on the quiz (3p per correctly answered item on each version), and their score on the training game to bring the payment up to £10.  
Training is divided into four blocks, and after each block, subjects are told that they can take a break if they wish. The study is implemented using Gorilla (@anwyl-irvine2020), a cloud-based research tool for behavioral studies. The study takes around one hour to complete.  
...
### Online training
**Instructions**:  After the estimation quiz pre-test, subjects are introduced to the game, as follows: 

_Imagine you are a researcher running a large range of experiments. All of the experiments are comparing a control group with a group receiving an experimental treatment (intervention) of some sort. You want to see whether the experimental group scores higher than the control group after the intervention/treatment. For our purposes, it doesn’t matter too much what the research topic is – the same principle should apply to many different types of experiments. For instance, you could be examining:_ 
...
* depression scores after an experimental therapy treatment versus standard treatment 
...
_Importantly, we expect the data in these trials to be normally distributed –  most data points will be close to the middle, and fewer data points at the extremes of very high or very low values. The onion shapes to the right are both normal distributions (the width of the shape represents the proportional number of data points in this population that have the value of the y-axis). The boxplot in the centre of these onion shapes details further information about the distribution. The top image on the right compares the distributions separately, the bottom makes a side-by-side comparison. Throughout this game, you will see the bottom image when receiving feedback. If there is no effect of the experimental treatment, the overall distribution of the data might look like the figures to the right, with the distribution of scores from the experimental group looking very similar to those in the control group:_  
_A large number of trials have been run on the experimental interventions we described-- and you will get to see the data. In any given trial, there is a 50% chance you will see data from an experimental treatment (intervention), which has an effect size of .3, and a 50% chance you will see data from an experimental treatment that has no effect. Your aim is to earn as many points as possible by correctly identifying which trials show data from a population in which there is a true effect, and in which there is no effect. Points earned on the game will be converted to pennies in the bonus payment, but we will not deduct payment for a negative score. _ 
_The goal is for you to get a sense of how big a sample you need to make this judgment. The game will include 80 trials in total, with breaks in between each block of 20 trials. Every trial will start by showing a few data points from each group. At that stage you can EITHER:_  
* judge whether there is a true effect or no effect, by pressing one of the buttons below the data, OR 
* you can wait a few seconds for the screen to automatically advance and show you more evidence (additional data points). 
_Collecting more evidence can make you more sure of your decision._   

The subject is then shown some example stimuli, that demonstrate what the stimuli look like, and the reward schedule is explained. 
Subjects are also told: _Please note: this study does NOT involve any deception. The feedback you get will be accurate, even if it is sometimes surprising._  
...
The main dependent measure from the training trials is response latency, which is converted to array size, as described above. For data analysis, array size (10, 20, 40, 80, 160, 320) is converted to array index, ranging from 1 to 6.  In addition, stored with each array is the true  effect size in the population on that trial (Null or 0.3), coding of each response as correct (1) or incorrect (0), the observed effect size on that trial, and the log likelihood of a true vs null effect obtained from the observed data. 

In a pilot study  with this task (see below), the reward schedule was simply 10 points for a correct response and -10 for an incorrect response. One drawback of this approach is that the only incentive to respond before the final display is to minimize time in the experiment. In the current version, we therefore will include a penalty for waiting for larger sample sizes, with a potential for larger rewards at small sample sizes (reward going from 6 to 1 pence for array index 1 to 6, with deduction of 4 pence for an error). This will make the subject's decision more life-like, since there is a cost to using larger vs smaller samples. In contrast to the pilot session, we will also convert the total points earned into a financial bonus payment.  
...
In our pilot data, no subjects scored above 80% overall on the training task, but it is anticipated that the updated version of training (showing the mean bar) will be easier, and it is possible that some will obtain scores that are so high that no learning effect will be detectable. If there are subjects who score above 90% in the first block of training, they will be removed from the main analysis and treated separately. It is predicted that these would be people with a good prior understanding of sample size effects, and accordingly should be superior to other subjects on the S-items from the quiz at both pre-test and post-test.  

For the regression analysis, we use an index of learning which corresponds to the difference in mean array index between first and last blocks. We will exclude any subjects with a mean array size greater than 5 in the first block, since they will show a ceiling effect that will make the predictor variable insensitive. 



## Data analysis
_Hypothesis 1_: Responses will reflect overconfidence in small samples.
If this hypothesis is not confirmed, then this would undermine the rationale of the study, so this may be regarded as a form of positive control. This hypothesis will be tested using data both from the Pretest estimation quiz and the training task. On the pretest estimation quiz, it is expected that the majority of subjects will fail to select the correct response on items that assess awareness of the importance of sample size, and when they do so, will either respond that sample size is not important, or, when asked to select an optimal sample size, will select one that is smaller than required.  
...
_Hypothesis 2_: Overconfidence in small samples will decline with training.  
...
_Hypothesis 3_: Generalisation of learning  
Here the prediction is that individual differences in learning on the training task will predict improvement on the estimation quiz, specifically for items that are designed to assess sampling variability blindness. Individual differences in learning will be characterised by arraydiff, the difference in array index at the point of response for last and first blocks. 
The prediction will be tested using linear regression:  
_postS ~ preS + arraydiff_,    
where postS and preS are posttest and pretest scores on S-items, and arraydiff is the difference in mean array index between the last and first block of training. This simple approach to analysis was settled upon after using simulated data to conduct power analysis comparing linear regression versus a linear mixed models approach. The script for simulation is available on OSF (https://osf.io/rnkhq/?view_only=b0fcb097cc3044aaa942f15136441c49).

Further exploratory analysis will be used to scrutinise the response profiles on the training task for those subjects who do show learning, as well as their self-report of strategies, with the aim of determining which cues they rely on when making a response (see Exploratory analyses, below). 
...
## Stopping rule  
We will do an initial check after testing 30 subjects to see whether the predictions from Hypothesis 1 are supported, indicating that subjects on average show sampling variation blindness. If not, then we will halt data collection, as the basic premise of the study would not be supported.  

Assuming the study continues, we will stop and check results after each increment of 30 subjects. Because power depends crucially on the proportion showing learning, and the increase in array index with learning, both of which are unknown at this point, we will adopt a Bayesian stopping rule at sequential stages in data collection, starting with a sample of 60, and then increasing to 90 and 120 if necessary to meet our prespecified criteria. We will compute Bayes Factors for the t-test analysis testing hypothesis 2, and the linear regression for testing hypothesis 3.  While Bayes Factors fall between 1/3 or 3 we will continue with data collection. If the Bayes Factors yield evidence in favor of the alternative or null hypothesis for both analyses, we will stop data collection.  


## Exploratory analyses  
As well as the planned analyses using array index as a dependent variable, the pattern of responses on the training task will be explored to see whether individual subjects adopt specific strategies. Accuracy scores will be used to compute signal detection indices, d prime and beta, for each block of training, based on the number of hits (respond yes when there is a true effect); the number of misses (respond no when there is a true effect), the number of false alarms (respond yes when there is a null effect) and the number of correct negatives (respond no when there is a null effect). Calculations of these indices will be conducted using the _dprime_ function from the _psycho_ package (version 0.5.0; Makowski,2018). The beta values will allow us to detect cases of response bias, i.e. a tendency to always respond 'blue=pink' or 'blue>pink'.  
...
Of course, a problem for subjects is that they cannot estimate log likelihood directly from the information they are presented with. Our primary analysis assumes that, on the basis of feedback, subjects may simply learn that they are more likely to be accurate if they wait to see more data, in which case, they would base their judgment on observed effect size, once the array index gets to 5 or above. By observing the pattern of responses in individuals, we may be able to determine if a subject adopts a more specific strategy, and if so, whether they place reliance on observed effect size or log likelihood. 



## Simulated data for sample size determination
To simulate data, we used our best judgment combined with insights from the pilot testing to explore feasible scenarios. We started by assuming that at the start of training, on each trial, subjects will select an array size with a fairly flat distribution of probabilities of array indices as follows: index 1 = .1, index 2 through index 5 = .2 each,index 6 = .1. The response made by the subject is determined by the observed effect size of the sample in that array for the actual stimuli used in training: Blue > Pink (True effect) if observed effect size is greater than .15, and Blue = Pink (Null effect) if the observed effect size is less than this. By the final block, a proportion (p) of subjects will have learned the benefit of using larger arrays. Whether a subject falls into that proportion of learners is simulated using a variable, L, which is latent, in the sense that it affects performance on observed measures, but is not itself detectable. L is either 0 or 1. 

Where L is equal to one (i.e. the subject is a learner), the array index at which a selection is made is increased by a variable A (1 or 2) points. (If the index value exceeds 6, it is truncated to 6). A simulated sample of subjects is created by applying these rules to the stimulus set used in training, with response on each trial being then scored as correct or incorrect.

For Hypothesis 1 we simply test whether the mean array index for the whole sample on block 1 is less than 5. The tendency for subjects to start by selecting array indices under 5 is built into the simulation, with expected array index mean of 3.5. The difference from mean of 5 is detectable with as few as 30 subjects (power = 100%). As noted above, if the mean array index in the first block was _not_ appreciably less than 5, this would indicate that these subjects did not suffer from sampling variation blindness, and so the rationale for the training would be undermined.  

Hypothesis 2 focuses on whether there is evidence of learning to wait for larger arrays in the course of training, with larger arrays predicted for the last block than for the first block. The sample size (N subs) was varied from 60, to 90, to 120. The proportion of subjects who learn (prop.learned) could take values of 0, .25, .33 or .5, and  the mean increase in array size for those who learn, A, was either 1 or 2. `r tabnumber <- tabnumber+1` Table `r tabnumber` shows the power for detecting an overall change in mean array size from block 1 to block 4, using a matched-pairs one-tailed t-test, with alpha of .0322 (.1/3, for one-tailed test administered at each of three sample sizes). A one-tailed test is used because the prediction is directional. These figures were derived from 500 iterations of the simulation.  

The case where no subjects learn corresponds to a test of the null hypothesis, and so here the power value corresponds to the false positive rate. As expected, this corresponds to .03. When only 25% of subjects learn, power is below .8, particularly when learning increases array size by only one index point. When 50% of subjects learn, power is very high (above .8) at all sample sizes, even when the gain in array index is only 1 point. When 33% of subjects learn, and there is a 2-point gain in array index for those who learn, the power is above .9 for all sample sizes, but is between .58 and .68 if there is only a 1-point gain in array index.  As might be expected, the combination of a small proportion of learners with a small effect of learning is hard to detect consistently. Nevertheless, we have a good chance of detecting learning if at least 1 in 3 subjects learn, particularly if learning is associated with a gain of 2 points in array index. 



```{r powertable,include=TRUE,echo=FALSE}
bigsummary<- read.csv("bigsummary_A12flat_pgood0.5_0.66_props0_0.25_0.33_0.5_N60_90_120.csv")
bigsummary<-bigsummary[,c(1:4,8,6,9:11)]
nsubvals<-c(60,90,120)
plearnvals<-c(0,.25,.33,.5)
quizgoodvals<-c(.5,.66) #all subjects start with mean proportion correct on quiz S-items of .33. Those who show learning on training task increase to either .5 or .66. (Equivalent to increase by 1 or 2 items)
thisrow<-0
alpha <- .0166*2 #sequential stopping means need divide .05 by 3, but then note we have one-tailed test
powersummary<-data.frame(matrix(NA,ncol=9,nrow=length(nsubvals)*length(plearnvals)*length(quizgoodvals)*2))
colnames(powersummary)<-colnames(bigsummary)
wc<-which(colnames(bigsummary)=='p.t.array')
...
#columes 2-5 are parameters that are varied:
# nsub
# plearn - proportion who learn
# A - increase in array size in learners
# quiz.pgood - proportion correct on quiz at time 2 in learners

#column p.t.array is power for testing change in mean array from block1 to block4 (1-sided t-test)
...
# column lmarraydiff.p is power of regression coefficient for arraydifference (block 4 minus block 1) as predictor quiz posttest.


#Make table for learning effect in task
#For learning effect on task, we just average power across levels of quiz success
learntab <- aggregate(powersummary$p.t.array,by=list(powersummary$nsub,powersummary$plearn,powersummary$A),FUN=mean)
table1<-data.frame(matrix(NA,nrow=10,ncol=4))
colnames(table1) <- c('Parameters','N=60','N=90','N=120')
table1[1,]<-c('Learning gain (A) = 1','.','.',',')
table1[6,]<-c('Learning gain (A) = 2','.','.',',')
table1[2,1] <- '_plearn = 0'
table1[3,1] <- '_plearn = .25'
table1[4,1] <- '_plearn = .33'
table1[5,1] <- '_plearn = .50'
table1[7,1] <- '_plearn = 0'
table1[8,1] <- '_plearn = .25'
table1[9,1] <- '_plearn = .33'
table1[10,1] <- '_plearn = .50'
...
for (A in 1:2){
  for (pl in 1:4){
    ...
    table1[thisrow,2:4]<-round(learntab[thatrow:(thatrow+2),4],2)
  }
}
...
ftab <- set_caption(ftab, paste0("Table ",tabnumber,": Power for comparison of array index for block 4 vs block 1, in relation to gain in array size for learners (A), and proportion of learners (plearn)"))
ftab<-autofit(ftab)
ftab
...
To address Hypothesis 3, quiz data were simulated using the binomial distribution, by assuming the mean number of S-items correct on the pretest is 2/6 (.33), improving by either one item (3/6 = .5) or two items (4/6 = .66) for the subgroup of subjects who showed learning on training (where the latent variable, L = 1).  

`r tabnumber <- tabnumber+1`Results are shown in Table `r tabnumber`, again looking at scenarios with 60, 90 or 120 subjects, where the proportion showing learning ranges from 0, .25, .33 and .5, and where the array index improvement in learners is either 1 or 2. One other parameter is considered for the quiz data, namely the amount of improvement in quiz scores seen in learners - a mean gain of either 1 quiz item, or 2 quiz items.  

```{r powerquiz,include=TRUE,echo=FALSE}
#summary for quiz improvment
quizsum<-cbind(powersummary[1:24,c(2,3,5,7,8,9)],powersummary[25:48,7:9])
colnames(quizsum) <- c('N subs','prop.learned','learned.quiz2','power1.quizdiff','power1.quizpre','power1.arraydiff','power2.quizdiff','power2.quizpre','power2.arraydiff')
quizsum[,4:9]<-round(quizsum[,4:9],2)
...
colnames(table2) <- c('Parameters','N=60','N=90','N=120')
...
ftab2 <- set_caption(ftab2, paste0("Table ",tabnumber,": Power for comparison of quiz pre and post test, inr relation to number of subjects (N), proportion of learners (plearn) and increase in quiz score in learners (Quiz gain)"))
ftab2<-autofit(ftab2)
ftab2

```
Power to detect any improvement in the group overall is generally poor if one just compares quiz pre-test and post-test items. This follows because only a proportion of subjects show any learning, and the relationship between learning and quiz post-test performance is imperfect. 


```{r powerLM,include=T,echo=F}
#Now do table 3 for power for regression
#Make table3 for prediction of quiz posttest from arraygain
#For learning effect on task, we just average power across levels of quiz success
LMtab <- aggregate(powersummary$lmarraydiff.p,by=list(powersummary$nsub,powersummary$plearn,powersummary$quiz.pgood,powersummary$A),FUN=mean)
table3<-data.frame(matrix(NA,nrow=20,ncol=4))
colnames(table3) <- c('Parameters','N=60','N=90','N=120')
...
table3[11:20,1]<-table3[1:10,1]
table3[1,]<-c('A = 1, quiz gain = 1','.','.',',')
table3[6,]<-c('A = 1, quiz gain = 2','.','.',',')
table3[11,]<-c('A = 2, quiz gain = 1','.','.',',')
table3[16,]<-c('A = 2, quiz gain = 2','.','.',',')
...
for (A in 1:2){
  ...
  for (pl in 1:4){
    ...
    thatrow<-thatrow+3
    if(pl==1){thisrow <- thisrow+1}
    table3[thisrow,2:4]<-round(LMtab[thatrow:(thatrow+2),5],2)
  }
  }
}
...
ftab3 <- set_caption(ftab3, paste0("Table ",tabnumber,": Power for  regression coefficient predicting posttest score on S-items from observed increase in array index on learning task. Power varies with number of subjects (N), proportion of learners (plearn), mean array index increase for learners (A), and quiz gain at posttest for learners (quiz gain)"))
ftab3<-autofit(ftab3)
ftab3
```
...
A more sensitive test is given with linear regression, where the focus is on predicting the quiz post-test score (S-items) from the pre-test score and the index of learning, i.e., the change in array index from block 1 to block 4. This analysis, then, can detect whether the amount of learning in the training session is predictive of improvement on the quiz.  

Once again, the combination of parameters determines power, with high power seen when at least 50% of subjects show learning, and when their quiz scores increase by 2 points. If only 33% of subjects show learning, the power is reasonable (close to or exceeding .8), provided the learning effect is strong, both in terms of increase in array index, and in terms of improvement on quiz items. Power to detect a one-point gain in quiz scores for learners is modest at best.  Once again, consideration of the case where no subjects show learning confirms that the type 1 error rate is well-controlled, so we can be confident that where positive findings are obtained, they are true effects.  
...
The first possible pattern is where neither the training task nor the quiz shows any improvement in sampling variation blindness. If this pattern were observed, we would consider developing a modified version of the task. There are various ways one could do this: by changing the visual display, or by changing the reward schedule. We will also conduct exploratory analyses on the dataset, to see if there is evidence that subjects use counterproductive strategies. It would, of course, also be possible to extend the training with additional trials, although this might require having more than one session. And another option would be to give more explicit instructions about the relationship between sample size and variation in observed effect size. In effect, the current study could then act as a baseline against which to evaluate other approaches to training.  

We may find significant improvements with training, but null or inconclusive results on the S-tiems of the quiz, this would indicate a lack of generalisation. If so, we will explore individual S-items in the quiz, to see whether there is any indication of improvement on a subset of items, and whether they have particular characteristics. We may, for instance, see evidence of selective improvement on items assessing understanding of the relationship between sample size and statistical power. Any hypotheses derived this way would need to be tested in another sample in a future study.
...
The training task was modified on the basis of results from pilot data collected from 21 participants recruited via Prolific. Data are available on OSF (https://osf.io/rnkhq/?view_only=b0fcb097cc3044aaa942f15136441c49). Overall performance on the pilot task was poor, with mean of only 65% correct, confirming the difficulty that subjects had in judging effect sizes from distributions. A score of 85% or above was observed in only 10% of all blocks x subjects. Pilot participants found the task reasonably straightforward but scrutiny of individual cases for evidence of response strategies suggested that only a few participants had started to wait longer to respond as the session proceeded, and others went in the opposite direction, selecting smaller arrays as the task proceeded.  In general, high performers were those who waited for large arrays from the outset, rather than those who changed strategy in the course of training. 

We therefore considered ways of making the training more effective. In the pilot task, the beeswarm displays for the two groups  had omitted the horizontal line corresponding to the mean, and each beeswarm faded out as the next one appeared, which meant it was less obvious that the distributions were cumulative. In addition, although the subjects were shown a demonstration of the task, they did not have any familiarisation trials, and it was felt they they might perform better if given the chance to try a few trials to become familiar with the pace of the task. In the current version of the task, the displays include a horizontal bar corresponding to the mean, and prior displays remain visible rather than fading them out, to emphasise that, within a trial, each set of points included the prior, smaller set. In addition, subjects will be given 4 practice trials before the training blocks, and the reward schedule is modified to impose a small cost for selecting larger arrays, to discourage subjects from just waiting until the last array as a default strategy. Instructions are also modified to stress that the stimuli were genuine, with no trick questions.

The pilot participants also completed the estimation quizzes that preceded and followed the training session. As noted above, modifications were made to wording of some quiz items for the current study on the basis of pilot testing to avoid confusing or over-complicated language. 
