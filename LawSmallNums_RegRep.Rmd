...
We will also examine the potential of this task to enhance participants' statistical reasoning by administering a pre- and post-training quiz, testing statistical reasoning using questions that assess sample size neglect.  The quiz also includes questions testing understanding of basic probability; these act as positive controls: if participants score below 66% on the positive control questions, then this would indicate they either have little training in statistics, or are not taking the task seriously.

Specific hypotheses are:  
_Hypothesis 1_: In line with early work by Tversky and Kahneman (1971), people will be overconfident about estimates based on small samples.  We predict that we will demonstrate this in the estimation quiz, where performance will be poorer on questions that focus on sample size neglect (S-items) than on questions testing understanding of probablity (P-items). 
  
_Hypothesis 2_: Repeated exposure to a training task with feedback can reduce overconfidence in small samples. Accordingly, we predict that, in the course of training, a proportion of participants will achieve higher levels of accuracy.  

_Hypothesis 3_:  Accuracy on the training task will be correlated with the array size at point of response, particularly in the final blocks, as participants become more adept at interpreting the information in larger arrays.  
  
_Hypothesis 4_: Learning will generalize beyond the training task. Individual differences in learning on the training task will predict improvement specifically on sample size neglect items (S-items) on the estimation quiz, but not affect performance on probability items (P-items). 
...
## Data analysis
_Hypothesis 1_:  Responses will reflect overconfidence in small samples. If this hypothesis is not confirmed, then this would undermine the rationale of the study, so this may be regarded as a form of positive control. This hypothesis will be tested using data from the Pretest estimation quiz,  where it is expected that the mean correct will be lower for S-items than for P-items. In addition, a qualitative analysis of error responses will be conducted to test the expectation that participants will either tend to respond that sample size is not important, or, when asked to select an optimal sample size, will select one that is smaller than required. 

_Hypothesis 2_: Accuracy will increase with training.  
We anticipate replicating the results of Pilot 2, showing improved performance, as reflected in earnings, with exposure to the training task. Originally, for Pilot 2, we also anticipated that the array index at the point of selection would be higher for the last than the first block of the training session, as participants learned that larger samples are more reliable, but this was not observed. As a subsidiary test of hypothesis 2, we will test for that predicted effect with our larger sample, but it is clear that improved performance can occur in the absence of a change in preferred array size, and so our primary focus will be on changes in the earnings measure.

_Hypothesis 3_: Increased accuracy will be associated with selection of larger array sizes as learning proceeds  
Based on Pilot 2 data, we postulate that although training did not lead participants to  wait for larger arrays, it was effective because it showed participants what reliable evidence looked like, and how it could change with sample size. We therefore predict that, as found in Pilot 2, there will be a significant association between mean array size and mean earnings, such that those who earn most will be those who select larger array sizes, and that this will be more pronounced in the last block than in the first block. 

_Hypothesis 4_: Self-reported strategy will be predictive of learning  
We will divide the self-reported responses into those that do and do not indicate awareness that larger sample sizes (or longer waiting) gives more reliable estimates. Consistent with Pilot 2, we predict that those who show such awareness will obtain higher earnings in the final block than those who do not. 

_Hypothesis 5_: Generalisation of learning  
Here the prediction is that individual differences in learning on the training task will predict improvement on the estimation quiz, specifically for items that are designed to assess sample size neglect. Individual differences in learning will be characterised by earn.diff, the difference in earnings for last and first blocks. 
...
The positive control P-items from the quiz will be scrutinised to check that the participants are competent in their knowledge of basic probability and motivated to respond accurately. If this is not the case, then it would undermine the interpretation of poor performance on S-items. 

## Stopping rule  
We will do an initial check after testing 30 participants to see whether the predictions from Hypothesis 1 are supported, indicating that participants on average show sample size neglect. If not, then we will halt data collection, as the basic premise of the study would not be supported. In effect, Hypothesis 1 acts as a positive control.

Assuming the study continues, we will stop and check results after testing 50 participants, and then after each increment of 25 participants. Because power depends crucially on the proportion showing learning, and the increase in percent correct with learning, both of which are unknown at this point, we will adopt a Bayesian stopping rule at sequential stages in data collection, starting with a sample of `r subn[1]`, and then increasing to `r subn[2]` and `r subn[3]` if necessary to meet our prespecified criteria. We will compute Bayes Factors for the t-test analysis testing hypothesis 2, and the linear regression for testing hypothesis 5.  While Bayes Factors fall between 1/3 or 3 we will continue with data collection. If the Bayes Factors yield evidence in favor of the alternative or null hypothesis for both analyses, we will stop data collection.  


## Exploratory analyses  
To complement the analysis of self-reported strategies (Hypothesis 4), the pattern of responses on the training task will be explored to see whether individual participants adopt specific strategies. Accuracy scores will be used to compute signal detection indices, d prime and beta, for each block of training, based on the number of hits (respond yes when there is a true effect); the number of misses (respond no when there is a true effect), the number of false alarms (respond yes when there is a null effect) and the number of correct negatives (respond no when there is a null effect). Calculations of these indices will be conducted using the _dprime_ function from the _psycho_ package (version 0.5.0; Makowski,2018). The beta values will allow us to detect cases of response bias, i.e. a tendency to always respond 'blue=pink' or 'blue>pink'.  

As shown in the power values in Figure `r keepfignumber`, with an effect size (Cohen's d) of .3, participants need to wait for an array of at least 80 per group (index 4) to have a reasonable chance of success (above 50%) for detecting a true effect, and would be well-advised to wait for the largest array (index 6), which would virtually guarantee success.  However, the trial by trial variation allows us to do a rather more sensitive analysis, testing how the evidence available on each specific trial is used.  
...
To address Hypothesis 5, quiz data were simulated using the binomial distribution, by assuming the mean number of S-items correct on the pretest is 2/6 (.33), improving by either one item (3/6 = .5) or two items (4/6 = .66) for the subgroup of participants who showed learning on training (where the latent variable, L = 1).  

`r tabnumber <- tabnumber+1`Results are shown in Table `r tabnumber`, again looking at scenarios with 50, 75 or 100 participants, where the proportion showing learning ranges from 0, .25, .33 and .5, and where the array index improvement in learners is either 1 or 2. One other parameter is considered for the quiz data, namely the amount of improvement in quiz scores seen in learners - a mean gain of either 1 quiz item, or 2 quiz items.  
...
ftab3 <- set_caption(ftab3, paste0("Table ",tabnumber,": Power for  regression coefficient predicting posttest score on S-items from observed increase in earnings on learning task. Power varies with number of participants (N), proportion of learners (plearn), and quiz gain at posttest for learners (quiz gain)"))
ftab3<-autofit(ftab3)
ftab3
```
...
At the suggestion of a reviewer, we conducted Pilot 3 with 20 participants, using an unpaced version of the task, in which on each trial, the participant was presented with a menu and required to select a sample size to view. A plot showing all sample sizes, similar to Figure `r keepfignumber+1`, was shown when feedback on accuracy was provided. A cost of one point was incurred with each increase in the selected array index, to prevent participants from always selecting the largest array size. No learning was observed in Pilot 3, which gave results similar to Pilot 1, and therefore this approach was abandoned.
