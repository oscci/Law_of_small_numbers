...
jpeg(plotname, width = 400, height = 300)
par(mfrow=c(1,2))
...
In our original pilot study  with this task (see below), the reward schedule was simply 10 points for a correct response and -10 for an incorrect response. One drawback of this approach is that the only incentive to respond before the final display is to minimize time in the experiment. We therefore included a penalty for waiting for larger sample sizes, with a potential for larger rewards at small sample sizes. This makes the subject's decision more life-like, since there is a cost to using larger vs smaller samples. In contrast to the pilot session, we also offered a bonus payment for those with high levels of accuracy.
...
In the training task, we predict that responses in the first block of training will tend to be non-optimal, made when the array size is small enough to make an error likely. The array index at which a response is made for all subjects will be compared with a value of 5, using a directional one-sample t-tests, with the prediction that most participants will make responses at an earlier array index than this.
...
where postS and preS are posttset and pretest scores on S-items, and arraydiff is the difference in mean array index between the last and first block of training. This simple approach to analysis was settled upon after using simulated data to conduct power analysis comparing linear regression versus a linear mixed models approach. The script for simulation is available on OSF (https://osf.io/rnkhq/?view_only=b0fcb097cc3044aaa942f15136441c49).

Further exploratory analysis will be used to scrutinise the response profiles on the training task for those subjects who do show learning, with the aim of determining which cues they rely on when making a response (see Exploratory analyses, below). 
...
## Stopping rule  
We will do an initial check after testing 30 subjects to see whether the predictions from Hypothesis 1 are supported, indicating that subjects on average show sampling variation blindness. If not, then we will halt the study, as the basic premise of the study would not be supported.  
...
To simulate data, we used our best judgment combined with insights from the pilot testing to explore feasible scenarios. We started by assuming that at the start of training, on each trial, subjects will select an array size with a fairly flat distribution of probabilities of array indices as follows: index 1 = .1, index 2 through index 5 = .2,index 6 = .1. The response made by the subject is determined by the observed effect size in that array for the actual stimuli used in training: Blue > Pink (True effect) if observed effect size is greater than .15, and blue = pink (Null effect) if the observed effect size is less than this. By the final block, a proportion (p) of subjects will have learned the benefit of using larger arrays. This is simulated using a variable, L, which is latent, in the sense that it affects performance on observed measures, but is not itself detectable. L is either 0 or 1. 

Where L is equal to one, the array index at which a selection is made is increased by a variable A (1 or 2) points. (If the index value exceeds 6, it is truncated to 6). A simulated sample of subjects is created by applying these rules to the stimulus set used in training, with response on each trial being then scored as correct or incorrect.

For Hypothesis 1 we simply test whether the mean array index for the whole sample on block 1 is less than 5. The tendency for subjects to start by selecting array indices under 5 is built into the simulation, with expected array index mean of 3.5. The difference from mean of 5 would be detectable with even a very small sample size. Power to detect this effect with the sample sizes simulated here (N = 60, 90 and 120) is 100%. As noted above, if the mean array index in the first block was not appreciably less than 5, this would indicate that these subjects did not suffer from sampling variation blindness, and so the rationale for the training would be undermined.  
...
#Make one table for learning effect in task and another for impact on quiz
...
learntab<-cbind(learntab[1:12,c(1,2,4)],learntab[13:24,4])
colnames(learntab)<-c('N subs','prop.learned','power_1','power_2')
learntab[,3:4]<-round(learntab[,3:4],2)
write.csv(learntab,'learntab.csv',row.names=F)
ftab <- flextable(learntab)

ftab <- set_caption(ftab, paste0("Table ",tabnumber,": Power for comparison of array index for block 4 vs block 1"))
ftab
...
Quiz data were simulated using the binomial distribution, by assuming the mean number of S-items correct on the pretest is 2/6 (.33), improving by either one item (3/6 = .5) or two items (4/6 = .66) for the subgroup of subjects who showed learning on training (where the latent variable, L = 1).  

`r tabnumber <- tabnumber+1`Results are shown in Table `r tabnumber`, again looking at scenarios with 60, 90 or 120 subjects, where the proportion showing learning ranges from 0, .25, .33 and .5, and where the array index improvement in learners is either 1 or 2. One other parameter is considered for the quiz data, namely the amount of improvement in quiz scores seen in learners - a mean gain of either 1 quiz item (average final score of .5), or 2 quiz items (average final score of .66).  
...
ftab <- flextable(quizsum)

ftab <- set_caption(ftab, paste0("Table ",tabnumber,": Power for (a) change in quiz score (quizpre); (b) regression coefficient predicting quiz gain on S-items from increase in array index on learning task (arraydiff). Power varies with number of subjects (N subs), proportion of learners (prop.learned), array index increase for learners (power1 vs power2), and quiz accuracy at posttest for learners (learned.quiz2)"))
ftab
```
...
The first possible pattern is where neither the training task nor the quiz shows any improvement in sampling variation blindness. If this pattern were observed, we would consider developing a modified version of the task. There are various ways one could do this: by changing the visual display, or by changing the reward schedule. We will also conduct exploratory analyses on the dataset, to see if there is evidence that subjects use counterproductive strategies. It would, of course, also be possible to extend the training, although this might require having more than one session. And another option would be to give more explicit instructions about the relationship between sample size and variation in observed effect size. In effect, the current study could then act as a baseline against which to evaluate other approaches to training.  
...
## Appendix 2: Summary data from 21 pilot subjects on an earlier version of the task  
Results from the quiz are proportion correct for S- and P-items for each subject at pretest and post-test. Results from the 4 blocks of training items are shown as number of Correct Negatives (NcorrN), Hits (Nhit), Misses (Miss), and False Positives (FP), with the block number as subscript. Dprime and beta are shown for each block ,as well as proportion correct, and the mean array size at the point of response. 

```{r controltab,include=T,echo=F}
pilotsum <- read.csv("pilot21summary.csv")
selectcols <- c(2:5,10:13,18:21,26:29,34:37)
pilotsum[,selectcols]<-round(pilotsum[,selectcols],2)
flextable(pilotsum[,1:13])
flextable(pilotsum[,c(1,14:21)])
flextable(pilotsum[,c(1,22:29)])
flextable(pilotsum[,c(1,30:37)])
```
