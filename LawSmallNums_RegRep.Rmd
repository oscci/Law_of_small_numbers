...
On the training task, the independent variable is block: there are 4 blocks each of 20 items, but our focus is on comparing blocks 1 and 4. The dependent variables are (a) mean array index per block, 1 to 6, corresponding to the array size (10, 20, 40, 80, 160, 320 per group) at which the response is made, and (b) earnings in the game, which reflects both proportion correct for each block, and ability to select the optimal array size at which to respond (see below).
...
We will also exclude from the main analysis any subjects who score 5 or 6 correct on S-items in the pretest quiz, as they would be people with a good prior understanding of sample size effects, so would be unlikely to show learning gains. We will check whether that assumption holds, by comparing performance of this subgroup with that of the rest of the sample on Block 1 of the training task.  We will recruit further subjects to substitute for these in the main analysis, regardless of how they perform.  
...
This prediction will be tested by assessing whether array index at the point of selection is higher for the last than the first block of the training session, using a one-tailed t-test. In addition, it is anticipated that within each block there will be a positive correlation between earnings (reflecting proportion correct and identification of the optimal array) and array index, indicating that subjects use the additional information in larger sample sizes to guide their responding.

_Hypothesis 3_: Generalisation of learning  
Here the prediction is that individual differences in learning on the training task will predict improvement on the estimation quiz, specifically for items that are designed to assess sampling variability blindness. Individual differences in learning will be characterised by earn.diff, the difference in earnings for last and first blocks. 
The prediction will be tested using linear regression:  
_postS ~ preS + earn.diff_,    
where postS and preS are posttest and pretest scores on S- quiz items, and earn.diff is the difference in earnings between the last and first block of training. This simple approach to analysis was settled upon after using simulated data to conduct power analysis comparing linear regression versus a linear mixed models approach. The script for simulation is available on OSF (https://osf.io/rnkhq/?view_only=b0fcb097cc3044aaa942f15136441c49).
...
Of course, a problem for subjects is that they cannot estimate log likelihood directly from the information they are presented with. However, they can estimate the amount of overlap between the blue and pink samples, which is predictive of log likelihood.  Our primary analysis assumes that, on the basis of feedback, subjects may simply learn that they are more likely to be accurate if they wait to see more data, in which case, they would base their judgment on observed effect size, once the array index gets to 5 or above. By observing the pattern of responses in individuals, we may be able to determine if a subject adopts a more specific strategy, and if so, whether they place reliance on observed effect size or sample overlap. 
...
bigname<-"powsummary_1000_A12_pgood0.5_0.66_props0_0.25_0.33_0.5_N50_75_100.csv"
