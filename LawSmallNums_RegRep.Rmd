...
#16052021 - Dietrichson2021 reference - mean effect size in education interventions = .3 (also updated bibtext)
...
## Subjects
Our subject inclusion criteria include: (1) age of 18 years or over, (2) have studied life or social sciences for at least one term at undergraduate (bachelor’s degree) level. We will recruit up to 100 subjects via the online research platform Prolific (www.prolific.co) and social media platforms. The maximum sample size is determined from a power calculation (see section on Simulated data for sample size determination, below) indicating that this is sufficient to detect the case when 33% or more of subjects shift from reliance on small arrays at start of training to arrays that are meaningfully larger by end of training. 

## Procedure  
After providing informed consent, subjects will complete the estimation quiz and judgment task online at a time and place of their choosing, followed by an alternative form of the post-test estimation quiz. Subjects are paid £7.50 for their time, but can earn a further bonus linked to the number of correct items on the quiz (3p per correctly answered item on each version), and their score on the training game to bring the payment up to £12.  
...
_You can earn rewards on this game for correct responses; these will be converted to pennies and added to your payment at the end of the game. For each correct response you are given 4p, and for each wrong response you lose 4p. Don't worry, your basic payment for this session is guaranteed, even if you end up with negative points._  

_You can also earn bonus points. On each trial, there is an optimal array size: this is the smallest array size where the evidence from the blue and pink dots is clear enough to either decide the means of the two groups are the same, or that they are different, with low likelihood of error.  As there's an element of chance in how the arrays were created, the optimal array size won't always be the same, but the more experience you have with the game, the easier you should find it. 
Every time you make a correct response at the optimal array size, you will earn a bonus of 2p._  
...
In a pilot study  with this task (see below), the reward schedule was simply 10 points for a correct response and -10 for an incorrect response. One drawback of this approach was that the only incentive to respond before the final display was to minimize time in the experiment. In the current version, we therefore encourage more thoughtful responding by explaining the idea of an optimal array size - which is the smallest array size at which the odds in favour of either the null or true hypothesis reaches 40:1 (absolute log likelihood of 3.68).  In contrast to the pilot session, we will also convert the total points earned (4p for correct, -4p for error, plus bonus) into a financial bonus payment.  
...
Hypothesis 2 focuses on whether there is evidence of learning to wait for larger arrays in the course of training, with larger arrays predicted for the last block than for the first block. The sample size (N subs) was varied from `r subn[1]`, to `r subn[2]`, to `r subn[3]`. The proportion of subjects who learn (prop.learned) could take values of 0, .25, .33 or .5, and  the mean increase in array size for those who learn, A, was either 1 or 2. `r tabnumber <- tabnumber+1` Table `r tabnumber` shows the power for detecting an overall change in mean array size from block 1 to block 4, using a matched-pairs one-tailed t-test, with alpha of .0322 (.1/3, for one-tailed test administered at each of three sample sizes). A one-tailed test is used because the prediction is directional. These figures were derived from 500 iterations of the simulation.  

The case where no subjects learn corresponds to a test of the null hypothesis, and so here the power value corresponds to the false positive rate. As expected, this is close to to .03. At our smallest sample size (`r subn[1]`), even when only 25% of subjects learn, power is above .8, even for the case where learning increases array size by only one index point. 


```{r powertable,include=TRUE,echo=FALSE}
bigname<-"bigsummary500_A12nu_pgood0.5_0.66_props0_0.25_0.33_0.5_N50_75_100.csv"
...
```
Power to detect any improvement in the overall sample is generally poor if one just compares quiz pre-test and post-test items, except where the gain in quiz score is at least 2 points and at least 33% of subjects learn. This follows because only a proportion of subjects show any learning, and the relationship between learning and quiz post-test performance is imperfect. 
...
As shown in Table `r tabnumber`, the combination of parameters determines power, with high power seen when at least 33% of subjects show learning, and when their quiz scores increase by 2 points. Power to detect a one-point gain in quiz scores for learners is modest for a sample size of 50, but reasonable to high with 100 subjects.  Once again, consideration of the case where no subjects show learning confirms that the type 1 error rate is well-controlled, so we can be confident that where positive findings are obtained, they are true effects.  
