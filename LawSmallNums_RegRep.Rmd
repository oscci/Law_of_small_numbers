...
Even statistically-trained people show 'sample size neglect', i.e. they underestimate how variability of mean estimates declines with sample size. We consider whether judging whether simulated datasets come from different populations can help people overcome this bias. Before and after training with simulated datasets, subjects are given parallel forms of a 12-item quiz testing statistical knowledge, including six items testing sample size neglect. We hypothesise that the training will counteract sample size neglect in some subjects, and those who show learning will also show improvement on quiz items relating to sample size neglect.  
...
On the training task, the independent variable is block: there are 4 blocks each of 20 items, but our focus is on comparing blocks 1 and 4. The dependent variables are mean array index per block, corresponding to the array size at which the response is made, and proportion correct for each block.  
On the quiz, the independent variable is time (pre-training or post-training) and the dependent variable is proportion correct for S-items and P-items.  
...
Data from the training trials are saved as response latency, which is converted to array size, as described above. For data analysis, array size (10, 20, 40, 80, 160, 320) is converted to array index, ranging from 1 to 6.  In addition, stored with each array is the true  effect size in the population on that trial (Null or 0.3), coding of each response as correct (1) or incorrect (0), the observed effect size on that trial, and the log likelihood of a true vs null effect obtained from the observed data. The latter two variables could potentially be used in exploratory analyses when evaluating which cues subjects base their responses on.
...
A second pilot study, Pilot 2, was run with 30 subjects on the new version of the training task (without the quiz).  This confirmed that not only was performance improved relative to Pilot 1, but also there was evidence of learning in terms of the percentage correct measure. The mean array index was numerically larger for the last vs the first block, but the difference was not statistically significant. However, for the final block only, there was a significant correlation between mean array index and percent correct, and several subjects reported that they learned to wait for larger arrays in the course of training, suggesting the task was effective in achieving its aim. Results from Pilot 2 are also available on OSF. 
